...
export function Navbar() {
  ...
  const logoElement = (
    <div className="relative w-6 h-6 flex items-center justify-center">
      ...
      <span className="absolute text-xs font-bold text-white">N</span>
    </div>
  )

  const navLinksData = [
    { label: "How It Works", href: "#services" },
    { label: "The N.E.X.U.S. Difference", href: "#testimonials" },
  ]
  ...
  const loginButtonElement = (
    <button
      type="button"
      onClick={() => {
        window.dispatchEvent(new CustomEvent("nexus-sphere-activate"))
        document.getElementById("nexus-sphere")?.scrollIntoView({ behavior: "smooth", block: "center" })
      }}
  ...
  const signupButtonElement = (
    <div className="relative group w-full sm:w-auto">
      ...
      ></div>
      <Link href="/download" className="relative z-10 inline-flex items-center justify-center px-4 py-2 sm:px-3 text-xs sm:text-sm font-semibold text-white bg-gradient-to-br from-blue-400 to-blue-600 rounded-full hover:from-blue-500 hover:to-blue-700 transition-all duration-200 w-full sm:w-auto">
        Download
      </Link>
    </div>
  )
