  ...
  return (
    ...
    >
      ...
      <div className="absolute left-1/2 top-[40%] -translate-x-1/2 -translate-y-1/2">
        <div className={`h-44 w-44 rounded-full border border-blue-400/30 bg-[radial-gradient(circle_at_35%_30%,rgba(255,255,255,0.2),rgba(37,99,235,0.2)_30%,rgba(0,0,0,0.9)_72%)] shadow-[0_0_45px_rgba(37,99,235,0.12)] transition-transform duration-300 ease-out sm:h-56 sm:w-56 ${mode === "workflow" ? "animate-[heartbeat_1.15s_ease-in-out_infinite]" : "group-hover:scale-105"}`} />
      </div>
      <div className={`absolute left-1/2 top-[40%] h-60 w-60 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10 transition-transform duration-700 ease-out sm:h-72 sm:w-72 ${mode === "workflow" ? "rotate-45 scale-110" : "rotate-[18deg] group-hover:rotate-12"}`} />
      <div className={`absolute left-1/2 top-[40%] h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full border border-blue-300/10 transition-transform duration-1000 ease-out sm:h-80 sm:w-80 ${mode === "workflow" ? "-rotate-45 scale-105" : "-rotate-[24deg] group-hover:-rotate-12"}`} />
      {mode === "intro" && <div className="absolute inset-x-3 bottom-5 min-h-14 text-center font-mono text-sm leading-6 text-transparent bg-gradient-to-r from-blue-300 via-cyan-200 to-blue-500 bg-clip-text sm:inset-x-8 sm:text-base">{typedText}<span className="ml-0.5 animate-pulse text-blue-300">▋</span></div>}
      {mode === "workflow" && <div className="absolute inset-x-3 bottom-5 min-h-14 text-center font-mono text-sm leading-6 text-transparent bg-gradient-to-r from-blue-300 via-cyan-200 to-blue-500 bg-clip-text sm:inset-x-8 sm:text-base">{workflowStates[statusIndex]}</div>}
    </button>
  )
}
